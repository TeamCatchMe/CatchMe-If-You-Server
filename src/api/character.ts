...
router.get("/", auth ,async (req: Request, res: Response) => {
  try {
    ...
    if ( character['activity'].length == 0 ) {
    ...
    .find({user_id : req.body.user.id}, { activity : true })
    .sort({ activityYear : -1, activityMonth : -1 });
    
    // 전체 캐릭터가 쓴 게시글의 총 개수를 구합니다.
    for (var i = 0; i < allActivities.length; i++) {
      allActivitiesCount += allActivities[i]["activity"].length
    };
    // 캐릭터가 쓴 게시글의 총 개수를 구합니다.
    const characterActivitiesCount = character['activity'].length;
